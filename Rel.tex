...
\paragraph{Relations as Kleisli arrows}
...
\paragraph{Relations as spans}
On the other hand the same relation $R$ defines a subset of $X \x Y$
by $\{~(x,y) ~|~ x \sim_R y~\}$. In $\Set$, such a subset is a
\emph{span} over $X$ and $Y$:
\[\bfig
\Atriangle/->`->`{}/[R`X`Y;p`q`]
...
\efig\]
%
We leave out the details that this indeed forms a bicategory.

Finally, note that the
correspondence between spans and relations is an equivalence rather
than an isomorphism, as there might be many spans representing
(tabulating) the
same relation. However, all such spans are equivalent via suitable
2-cells. Thus, strictly speaking $\Rel$ is the quotient
